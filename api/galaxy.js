 ...
 * constellations. When watsonx is unconfigured or failing, the platform's tagged
 * embedding lanes (NVIDIA NIM, Vertex AI, OpenAI) position the map instead, and the
 * payload's `model` names the space that actually served.
 *
 ...
 *                                   Embeds the query in the same vector space as the
 *                                   current snapshot, ranks published agents by cosine
 *                                   similarity against their stored vectors, returns
 *                                   the top matches.
 *
 * No mock path. Coordinates and rankings come from real embeddings; when no embedder
 * is configured at all the endpoint returns 503 with a machine-readable reason so the
 * viewer can explain the requirement instead of inventing a fake universe.
 */
...
export default wrap(async (req, res) => {
	...
	const embedders = agentEmbedders(cfg);
	if (!embedders.length) {
		return error(
			res,
			503,
	...
	if (req.method === 'POST') return handleSearch(req, res, embedders);
	return handleGalaxy(req, res, cfg, embedders);
});

// ── GET: the galaxy snapshot ────────────────────────────────────────────────

async function handleGalaxy(req, res, cfg, embedders) {
	...
	if (!job) {
		job = buildGalaxy(cfg, embedders, limit).finally(() => building.delete(limit));
...
// Build a fresh galaxy: load published agents, ensure their embeddings, project +
// cluster, name the constellations with Granite, persist the snapshot.
async function buildGalaxy(cfg, embedders, limit) {
	const agents = await loadPublishedAgents(limit);

	if (agents.length === 0) {
		const empty = { count: 0, dims: 0, model: embedders[0].model, clusters: [], agents: [] };
		await persistSnapshot(empty);
		return empty;
	}

	const { vectors, dims, model } = await embedWithFailover(embedders, agents);
...
// Embed the whole population with one embedder, walking the preference order when
// a lane fails. A map is only ever built from a single vector space: a lane that
// fails part-way is abandoned entirely and the next one embeds every agent.
async function embedWithFailover(embedders, agents) {
	let lastErr;
	for (const embedder of embedders) {
		try {
			return await ensureAgentEmbeddings(embedder, agents);
		} catch (err) {
			console.warn(`[galaxy] embedder ${embedder.model} failed: ${err?.message || err}`);
			lastErr = err;
		}
	}
	throw lastErr;
...
// The embedder whose space the served map lives in, so a query is compared
// against vectors from the same model. Falls back to the preferred embedder when
// no snapshot exists yet or its model is no longer configured.
async function snapshotEmbedder(embedders) {
	try {
		await ensureSnapshotTable();
		const [snap] = await sql`
			SELECT model FROM galaxy_snapshots WHERE agent_count > 0 ORDER BY created_at DESC LIMIT 1
		`;
		return embedders.find((e) => e.model === snap?.model) || embedders[0];
	} catch {
		return embedders[0];
	}
}

async function handleSearch(req, res, embedders) {
	...
	// Embed the query with the same model the agents were embedded with, so query
	// and agent vectors share a space.
	//
	// There is no provider failover to reach for here, and that is not an
	// oversight: the stored agent vectors live in one model's space, so a vector
	...
	const embedder = await snapshotEmbedder(embedders);
	let queryVec = null;
	let embedError = null;
	try {
	...
	const vectorsById = await readAgentVectors([...byId.keys()], { model: embedder.model });
	...
	return json(res, 200, { query, model: embedder.model, count: results.length, results, ranking: 'semantic' });
}
