...
export function renderPack({ brief, draft, slot, ledgerEntry = {}, rank = null, total = null, model = null, verification = null }) {
	...
	push(
		...
		'',
		`**Surface:** [\`${brief.key}\`](${url}) · **Stage:** drafted · **Slot:** ${slot.notBefore.slice(0, 16).replace('T', ' ')} UTC · **Announced externally:** never`,
		'',
		...
		'|---|---|',
		`| Pack id | \`${id}\` |`,
		`| Publish slot | ${slot.notBefore.slice(0, 16).replace('T', ' ')} UTC, jittered inside ${slot.windowMinutes} minutes by the scheduler |`,
