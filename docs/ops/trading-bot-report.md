...
| `realized PnL` | `realized_pnl_lamports` | Sum over every position the bot has ever held, in SOL. Net of the [three.ws trade fee](../pump-platform-fee.md): the buy fee sits inside the cost basis and the sell fee is subtracted from each closing leg, so a customer-owned bot's PnL is what it actually kept. |
| `deployed` | `entry_quote_lamports` | Total SOL that has ever gone through an entry, fee included, so PnL reads against the capital that actually worked. For a customer-owned bot that is the entry size plus the 1 percent trade fee; platform-owned fleets pay no fee and their two figures are the same. |
