...
A mirrored trade is a trade three.ws signs, so it carries the platform trade fee like every other one: 1 percent of the SOL spent on a buy, or of the minimum SOL out on a sell, transferred to the platform treasury inside the same transaction. It is built by the shared `buildTradeInstructions`, which is why the mirror cannot route around it, and it is skipped for platform-owned agents and wherever no fee wallet is configured. The rate, the basis, and what is not billed are in [platform trade fee](./pump-platform-fee.md). The copier path has no such fee: you sign that trade yourself, from your own wallet.
