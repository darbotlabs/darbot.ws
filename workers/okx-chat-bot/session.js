...
export function providerInstructions(detail = '', lanes = []) {
	...
	return [
		...
		...chain,
		'',
		'0. Payment-free: the anthropic-gateway lane on three.ws\'s own proxy',
		'   (/api/llm/anthropic/agents/<agent>, the free model chain). If it is in the',
		'   chain above and refused, `npm run okx:bot:gateway -- --verify` says why;',
		'   `-- --apply --rotate` re-mints its key; the API must be deployed with the',
		'   agent-scoped SDK path (`npm run deploy:gcp:full`). If it is missing from',
		'   the chain, redeploy the worker: `npm run okx:bot:deploy`.',
		...
		'4. Or point the gateway lane at a funded third-party Anthropic-wire-format',
		'   gateway instead (config only, pre-approved). OpenRouter serves one at',
		'   /api/v1/messages:',
		'   gcloud run services update okx-chat-bot --region us-central1 \\',
